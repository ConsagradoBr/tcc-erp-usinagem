...
def seed_database(
    num_clientes=500, num_orcamentos=800, num_lancamentos=1500, num_os=400
):
    print(
    ...
    for i in range(num_clientes):
        ...
        c = Cliente(
            ...
            municipio=choice(
                ["São Paulo", "Campinas", "Sorocaba", "Jundiaí", "Ribeirão Preto"]
            ),
    ...
    for i, lancamento in enumerate(lancamentos):
        if i < 50:
            lancamento.descricao = f"[ORC:CAP-{i+1:04d}] {lancamento.descricao}"
    ...
    for i in range(num_os):
        os = OrdemServico(
            ...
            servico=choice(
                ["Torneamento", "Fresagem", "Solda", "Corte", "Mandrilagem"]
            ),
...
def measure(label, func, warmup=True):
    ...
    worst = max(times)
    print(
        f"  {label:45s}  avg={avg*1000:7.2f}ms  best={best*1000:7.2f}ms  worst={worst*1000:7.2f}ms"
    )
    return avg
...
def old_orcamentos_resumo():
    ...
    orcamentos_ativos = {
        ...
        for o in aprovados
        if any(
            f"[ORC:{o.numero}]" in (lancamento.descricao or "")
            for lancamento in lancamentos_abertos
        )
    }
    result = {
        s: len([o for o in todos if o.status == s])
        for s in ["rascunho", "enviado", "aprovado", "reprovado", "cancelado"]
    }
...
def run_capacity_tests(client):
    ...
    results["financeiro_resumo"] = measure(
        "GET /financeiro/resumo",
        lambda: client.get("/financeiro/resumo", headers=auth_headers),
    )

    results["orcamentos_resumo"] = measure(
        "GET /orcamentos/resumo",
        lambda: client.get("/orcamentos/resumo", headers=auth_headers),
    )

    results["ordens_servico_resumo"] = measure(
        "GET /ordens-servico/resumo",
        lambda: client.get("/ordens-servico/resumo", headers=auth_headers),
    )

    results["dashboard_resumo"] = measure(
        "GET /dashboard/resumo",
        lambda: client.get("/dashboard/resumo", headers=auth_headers),
    )

    results["financeiro"] = measure(
        "GET /financeiro", lambda: client.get("/financeiro", headers=auth_headers)
    )

    results["orcamentos"] = measure(
        "GET /orcamentos", lambda: client.get("/orcamentos", headers=auth_headers)
    )

    results["ordens_servico"] = measure(
        "GET /ordens-servico",
        lambda: client.get("/ordens-servico", headers=auth_headers),
    )

    results["clientes"] = measure(
        "GET /clientes", lambda: client.get("/clientes", headers=auth_headers)
    )
    ...
    results["old_financeiro_resumo"] = measure(
        "  old /financeiro/resumo (Python)", run_old_fin, warmup=False
    )
    ...
    results["old_orcamentos_resumo"] = measure(
        "  old /orcamentos/resumo (Python)", run_old_orc, warmup=False
    )
    ...
    if (
        abs(old_fin["a_receber"] - new_fin["a_receber"]) < 0.01
    ...
    if (
        abs(old_orc["valor_total"] - new_orc["valor_total"]) < 0.01
        and abs(old_orc["valor_aprovado"] - new_orc["valor_aprovado"]) < 0.01
        and abs(old_orc["valor_aprovado_ativo"] - new_orc["valor_aprovado_ativo"])
        < 0.01
    ):

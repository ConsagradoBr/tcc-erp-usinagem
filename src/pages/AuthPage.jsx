...
export default function AuthPage() {
    ...
    const checkBootstrapStatus = async () => {
      try {
        ...
        if (err.code === "ERR_NETWORK" || err.message?.includes("Network Error")) {
          setError(
            "Não foi possível conectar ao servidor. Verifique se a API está rodando em http://localhost:5000"
          );
        } else {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      navigate(getDefaultAppRoute(user));
    } catch (err) {
      if (err.code === "ERR_NETWORK" || err.message?.includes("Network Error")) {
  ...
  return (
    <div className="flex min-h-screen w-screen flex-col overflow-auto bg-neutral-900 md:flex-row">
      <div className="relative flex min-h-[40vh] w-full items-center justify-center overflow-hidden bg-white md:w-1/2">
        <div className="pointer-events-none absolute inset-0 hidden overflow-hidden md:block">
          {randomGears.map((gear) => (
            ...
            >
              <GearIcon className="h-6 w-6 md:h-8 md:w-8" />
            </div>
          ))}
        </div>
        ...
        <img
          src={logoGif}
          alt="AMP Usinagem"
          className="relative z-10 w-full max-w-md select-none object-contain [mix-blend-mode:multiply] pointer-events-none"
          draggable={false}
        />

        <div className="absolute right-0 top-0 hidden h-full w-px bg-neutral-200 md:block" />
      </div>

      <div className="relative flex min-h-[60vh] w-full flex-col items-center justify-center bg-gradient-to-br from-orange-300 via-orange-400 to-orange-500 px-6 py-10 md:w-1/2 md:px-8">
        <div className="w-full max-w-md rounded-2xl border border-white/35 bg-white/20 px-6 py-10 shadow-xl backdrop-blur-sm sm:px-8">
          <div className="mb-7 flex justify-center">
            ...
            >
              ...
              <path d="M21 47c0-6.1 4.9-11 11-11s11 4.9 11 11" fill="#1c1c1c" />
            </svg>
          </div>
          ...
          {error && (
            <div className="mb-5 flex items-center gap-2 rounded-lg border border-red-300 bg-red-100/50 px-4 py-2.5 text-sm text-red-700">
              <svg
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            <div className="flex flex-col gap-1.5">
              <label
                htmlFor="email"
                className="text-[11px] font-bold uppercase tracking-[3px] text-neutral-700"
              >
                E-mail
              </label>
              <div className="relative flex items-center">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-white/70 bg-white/55 py-3 pl-10 pr-4 text-sm text-neutral-800 placeholder-neutral-400 outline-none transition focus:border-orange-700 focus:ring-2 focus:ring-orange-500/30"
                />
              </div>
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative flex items-center">
                ...
                <input
                  ...
                  placeholder="••••••••"
                  className="w-full rounded-xl border border-white/70 bg-white/55 py-3 pl-10 pr-11 text-sm text-neutral-800 placeholder-neutral-400 outline-none transition focus:border-orange-700 focus:ring-2 focus:ring-orange-500/30"
                />
                <button
                  type="button"
            ...
            <button
              type="submit"
              disabled={loading}
              className="mt-1 flex w-full items-center justify-center gap-3 rounded-xl border-2 border-neutral-800/65 bg-white/30 py-3 text-sm font-semibold tracking-wider text-neutral-900 shadow-sm transition-all duration-200 hover:border-neutral-900 hover:bg-white/50 hover:shadow-md active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-60"
            >
              {loading ? (
                <svg
